...
public class TetrisClient extends Thread {
    ...
    private String key, current, preview, hold, score;
    ...
    public void send() {
		...
		o.println(key + data + ";" + current + ";" + preview + ";" + hold + ";" + score);
    }
				...
				if(line.length() != 0)
				{
					...
					if(!checkKey.equals(key)&& parsedData.length > 1) {
						...
						try {
							...
							System.out.println("enemyscore :"+parsedData[5]);
							tetrisCanvas.preview.enemy_score = Integer.parseInt(parsedData[5]);
						}catch(Exception e) {
							System.out.println("클라이언트 문제 발생:" + e);
						}
					}
				}
			}
